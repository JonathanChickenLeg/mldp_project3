...
if st.sidebar.button('Predict Depression'):
    ...
    df_input['financial_stress'] = pd.to_numeric(df_input['financial_stress'])
    df_input['academic_pressure'] = pd.to_numeric(df_input['academic_pressure'])
    df_input['total_stress'] = df_input['academic_pressure'] + df_input['financial_stress']
    ...
    sleep_map = {
        ...
        '7-8 hours': 7.5,
        'More than 8 hours': 9,
        # 'Others' is not mapped, will result in NaN if selected
    }
    df_input['sleep_numeric'] = df_input['sleep_duration'].map(sleep_map)
    df_input['burnout_ratio'] = df_input['study_hours'] / (df_input['sleep_numeric'] + 0.1)
